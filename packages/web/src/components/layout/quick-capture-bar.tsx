...
import { Sparkles, ImageIcon, Mic, ArrowRight } from 'lucide-react';
...
export function QuickCaptureBar({ onSearchOpen }: QuickCaptureBarProps) {
    ...
    try {
      ...
      if (result.ok) {
        ...
        mutate(
          (key: unknown) =>
            typeof key === 'string' &&
            (key.startsWith('/fragments') || key === '/today'),
        );
      }
    } catch {
  ...
  return (
    <div
      data-capture-bar
      className="fixed bottom-4 lg:bottom-8 left-0 lg:left-64 right-0 px-4 md:px-8 z-40 pointer-events-none">
