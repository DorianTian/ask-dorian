...
export default function TodayPage() {
  ...
  const targetDateStr = useMemo(() => (isToday ? undefined : formatDateISO(targetDate)), [isToday, targetDate])
  ...
  const toggleRitual = async (id: string) => {
    ...
    mutate((key) => typeof key === "string" && key.startsWith("/today"), undefined, { revalidate: true })
  }
  ...
  const extendBlock = async (block: { id: string; type: "event" | "task"; end: number }) => {
    const newEnd = new Date()
    newEnd.setHours(Math.floor(block.end + 0.5), Math.round(((block.end + 0.5) % 1) * 60), 0, 0)
  ...
  const doneCount = dashboard?.tasks.scheduled.filter((t) => t.status === "done").length ?? 0

  // Current time position for timeline
  const nowHour = now.getHours() + now.getMinutes() / 60
  const nowLabel = now.toLocaleTimeString("en", { hour: "2-digit", minute: "2-digit", hour12: true }).toUpperCase()
  ...
  return (
    <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-8 space-y-8">
      ...
      <section className="relative overflow-hidden rounded-2xl p-6 bg-surface-dark border border-primary/20 shadow-2xl shadow-primary/5 group cursor-pointer">
        <div className="relative z-10 flex flex-col md:flex-row items-start justify-between gap-6">
          <div className="space-y-3 max-w-2xl">
            ...
            <p className="text-lg md:text-xl font-medium leading-relaxed text-text-main group-hover:text-white transition-colors">
              ...
              {scheduledCount > 0 && (
                <>
                  <span className="text-primary font-bold">{scheduledCount} tasks</span> to complete.{" "}
                </>
              )}
              {pendingCount > 0 && (
                <>
                  <span className="text-primary font-bold">{pendingCount} fragments</span> pending review.
                </>
              )}
          ...
          >
            ...
            <ArrowUpRight size={14} className={`transition-transform ${briefExpanded ? "rotate-90" : "group-hover/btn:translate-x-0.5 group-hover/btn:-translate-y-0.5"}`} />
          </button>
        </div>
        ...
        {briefExpanded && (
          <div className="relative z-10 mt-4 pt-4 border-t border-primary/10 space-y-3 text-sm text-slate-400 font-mono">
            <p className="flex gap-2"><span className="text-primary/70">{">"}</span> <span><span className="text-primary font-bold">{ritualItems.length} rituals</span> configured. {completedRituals} completed today.</span></p>
            {overdueCount > 0 && (
              <p className="flex gap-2"><span className="text-orange-400/70">{">"}</span> <span><span className="text-orange-400 font-bold">WARNING:</span> {overdueCount} overdue tasks need attention.</span></p>
            )}
            <p className="flex gap-2"><span className="text-primary/70">{">"}</span> <span>Focus recommendation: prioritize <span className="text-white bg-white/10 px-1 py-0.5 rounded border border-white/10">{timelineBlocks.find(b => b.status === "active")?.title ?? "deep work"}</span> block.</span></p>
            <div className="p-3 bg-black/40 rounded-xl border border-primary/20 mt-2">
              <p className="text-[10px] text-primary/70 mb-1 tracking-widest uppercase">// Synthesized directive</p>
              <p className="text-slate-200 text-sm">{doneCount < scheduledCount ? `Complete remaining ${scheduledCount - doneCount} tasks before end of day.` : "All scheduled tasks done. Great work — review fragments or plan tomorrow."}</p>
            </div>
          </div>
        )}
      ...
      {overdueCount > 0 && dateOffset <= 0 && (
        <div className="bg-red-500/5 border border-red-500/20 rounded-2xl p-4 space-y-2">
          ...
          {dashboard?.tasks.overdue.map((task) => (
            <div key={task.id} className="flex items-center justify-between gap-3 px-2 py-1.5 rounded-lg hover:bg-red-500/5">
              <div className="flex items-center gap-2">
                <button
              ...
              {task.dueDate && (
                <span className="text-[10px] font-mono text-red-500/60">{task.dueDate}</span>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="space-y-3">
            {ritualItems.length === 0 ? (
              <p className="text-xs text-slate-500 text-center py-8">{t("noRituals")}</p>
            ) : (
                  ...
                  <div className="flex-1">
                    <p
                      className={`text-sm font-bold transition-all ${
                        item.completed ? "text-slate-600 line-through" : "text-text-main"
                      }`}
                    >
                      {item.title}
                    </p>
                    {item.isFocus && (
                      <p className="text-[10px] text-slate-500 mt-1 font-mono uppercase tracking-wider">Focus Phase • 45m</p>
                    )}
                  </div>
                  {item.isFocus && (
                    <span className="text-[10px] bg-primary/20 text-primary px-2 py-0.5 rounded-full font-black uppercase tracking-tighter">Timeboxing</span>
                  )}
                </div>
              ))
            )}
          ...
          {showAddRitual ? (
            <div className="w-full border-2 border-primary/50 rounded-2xl p-4 space-y-3 bg-surface-dark/60">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-black uppercase tracking-widest text-primary">New Ritual</span>
                <button
                  onClick={() => { setShowAddRitual(false); setNewRitualTitle("") }}
              ...
              <input
                ...
                onKeyDown={(e) => { if (e.key === "Enter") addRitual(); if (e.key === "Escape") { setShowAddRitual(false); setNewRitualTitle("") } }}
              ...
              >
                {isAddingRitual ? <Loader2 size={14} className="animate-spin" /> : <PlusCircle size={14} />}
                Add Ritual
              </button>
            </div>
          ) : (
        ...
        <div id="timeline" className="lg:col-span-7 space-y-6">
          <div className="flex items-center justify-between px-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => { if (dateOffset !== 0) setDateOffset(0) }}
            ...
            return (
              ...
              <div className="relative" style={{ height: TL_HEIGHT }}>
                ...
                <div className="absolute left-0 top-0 bottom-0 w-[70px] z-10 border-r border-border-dark/50">
                  {timeLabels.map((h) => (
                    <span key={h} className="text-[10px] text-slate-600 font-black font-mono text-right block pr-3" style={{ position: "absolute", top: `${hourToPercent(h, TL_START, TL_END)}%`, transform: "translateY(-50%)", right: 0, paddingTop: h === 0 ? 12 : 0 }}>
                ...
                <div className="absolute left-[70px] right-4 z-20 flex items-center pointer-events-none" style={{ top: `${nowPct}%` }}>
                ...
                <div className="absolute left-[70px] right-4 top-0 bottom-0">
                  {timelineBlocks.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full gap-4">
                      <p className="text-xs text-slate-500 font-mono">{t("noEvents")}</p>
                      <div className="bg-primary/5 border-2 border-dashed border-primary/20 p-4 rounded-2xl flex items-center justify-center gap-3 hover:border-primary/50 transition-all cursor-pointer group/box active:scale-[0.99] w-full max-w-sm">
                        <Sparkles size={16} className="text-primary/50 group-hover/box:scale-110 transition-transform" />
                        <p className="text-[10px] font-black uppercase tracking-widest text-primary/50 italic">Drop task to timebox</p>
                      </div>
                    </div>
                  ) : (
                    <>
                      {timelineBlocks.map((block) => {
                        const topPct = hourToPercent(block.start, TL_START, TL_END)
                        const heightPct = Math.max(hourToPercent(block.end, TL_START, TL_END) - topPct, 4) // min 4% height
                          ...
                          >
                            {block.status === "completed" ? (
                              <div className="h-full bg-black/40 border border-border-dark/50 rounded-xl p-3 flex items-center justify-between opacity-60 hover:opacity-80 transition-all cursor-pointer">
                                <div>
                                  <p className={`text-xs font-bold text-slate-500 ${isShort ? "" : "mb-0.5"}`}>{block.title}</p>
                                  {!isShort && (
                              ...
                              <div className="h-full bg-primary/10 border border-primary/50 ring-1 ring-primary/20 rounded-xl p-3 flex flex-col justify-center shadow-[0_0_30px_rgba(16,185,129,0.15)] z-20 cursor-pointer overflow-hidden">
                                <div className="flex items-center justify-between gap-2">
                                  <div className="flex items-center gap-2 min-w-0 flex-1">
                                    ...
                                    <p className="text-sm font-bold text-text-main truncate">{block.title}</p>
                                    <span className="text-[9px] font-mono text-slate-500 uppercase shrink-0">{block.subtitle}</span>
                                  </div>
                                  <div className="flex gap-1.5 shrink-0">
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation()
                                        block.type === "task" ? completeTask(block.id) : dismissEvent(block.id)
                                      }}
                                    ...
                                    >
                                      <CheckCircle2 size={10} /> {block.type === "task" ? "Complete" : "Done"}
                                    </button>
                                    <button
                                      onClick={(e) => { e.stopPropagation(); extendBlock(block) }}
                              ...
                              <div className="h-full bg-surface-dark/40 border border-border-dark/50 rounded-xl p-3 flex items-center gap-3 hover:border-primary/30 transition-colors cursor-pointer">
                                <div>
                                  <p className="text-xs font-bold text-text-main">{block.title}</p>
                                  {!isShort && (
                                    <p className="text-[10px] text-slate-500 font-mono uppercase tracking-wider">{block.subtitle}</p>
                                  )}
                                </div>
                              </div>
                            )}
                      ...
                      <div className="absolute left-0 right-0 px-2 z-0" style={{ top: `${hourToPercent(Math.max(...timelineBlocks.map(b => b.end)) + 0.5, TL_START, TL_END)}%`, height: "6%", minHeight: 40 }}>
                        <div
                        ...
                        >
                          <Sparkles size={14} className="text-primary/50 group-hover/box:text-primary transition-colors" />
                          <span className="text-[10px] font-mono font-bold text-primary/50 group-hover/box:text-primary transition-colors tracking-widest uppercase">Insert_Fragment</span>
                        </div>
                      </div>
                    </>
                  )}
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-4 pb-24">
        {[
          { label: t("statFocus"), value: "—", trend: undefined, detail: "Focus tracking available in Phase 2. Track your deep focus sessions and flow states." },
          { label: t("statDeepWork"), value: "—", unit: "hrs", detail: "Deep work tracking coming soon. Measures uninterrupted work blocks of 90+ minutes." },
          { label: t("statTasksDone"), value: `${doneCount}/${scheduledCount}`, detail: `${doneCount} of ${scheduledCount} scheduled tasks completed today. ${overdueCount > 0 ? `${overdueCount} overdue.` : "No overdue tasks."}` },
          { label: t("statEnergyPeak"), value: "—", detail: "Energy tracking available in Phase 2. Identifies your peak productivity hours." },
        ].map((stat, i) => (
          <div
            key={i}
          ...
          >
            ...
            <div className="flex items-end gap-2">
              ...
              {stat.trend && <span className="text-[10px] text-primary font-black mb-1">{stat.trend}</span>}
            </div>
            {expandedStat === i && (
